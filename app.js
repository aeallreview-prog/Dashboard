...
function resetTemplateBuilder() {
  ...
  document.getElementById('tplPriceUnit').value = '';
...
function assembleDetailTemplate() {
  ...
  const priceUnit = document.getElementById('tplPriceUnit').value;
  const stock = document.getElementById('tplStock').value.trim();
...
function parseDetailTemplate(text) {
  const result = { title: '', width: '', length: '', height: '', price: '', priceUnit: '', stock: '', closing: '', includeLength: false };
  if (!text) return result;
  const isBlank = v => !v || /^\.+$/.test(v);

  text = String(text).replace(/\r\n?/g, '\n').trim();
  const dimensionFields = { 'กว้าง': 'width', 'ยาว': 'length', 'สูง': 'height' };
  // Accept labelled centimetre measurements, including decimals and blank template values.
  const dimensionPattern = /(กว้าง|ยาว|สูง)[ \t]*[:：]?[ \t]*(\d+(?:\.\d+)?|\.+)[ \t]*(?:cm\.?|ซม\.?|เซนติเมตร)?(?=[ \t]*(?:[,，/;|\n]|$|กว้าง|ยาว|สูง|ราคา))/gi;
  for (const match of text.matchAll(dimensionPattern)) {
    const field = dimensionFields[match[1]];
    if (!result[field] && !isBlank(match[2])) result[field] = match[2];
    if (field === 'length') result.includeLength = true;
  }

  // A missing divider must not turn sizes, prices or closing notes into a product name.
  const titleBlock = text
    .replace(/^No\.[ \t]*\d+[ \t]*/i, '')
    .split(/^[ \t]*-{3,}[ \t]*$|(?:^|\n)[ \t]*(?:ราคา[ \t]*[:：]|In Stock[ \t]*[:：]|\(ราคายังไม่รวมค่าจัดส่ง\)|สินค้าเป็นของสะสม|#)/im)[0];
  const title = titleBlock.split('\n').map(line => {
    const withoutDimensions = line.replace(dimensionPattern, '');
    if (withoutDimensions !== line || /^[ \t]*ขนาด[ \t]*[:：]?[ \t]*$/.test(line)) {
      return withoutDimensions.replace(/ขนาด[ \t]*[:：]?[ \t]*/g, '')
        .replace(/^[ \t,，/;|]+|[ \t,，/;|]+$/g, '').trim();
    }
    return line.trim();
  }).filter(Boolean).join('\n');
  if (!isBlank(title)) result.title = title;
